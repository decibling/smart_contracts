...
describe("DeciblingAuction", function () {
    let DeciblingNFT, deciblingNFT, DeciblingAuction, deciblingAuction, token, accounts, owner, bidder1, bidder2, platformFeeRecipient;
    ...
    beforeEach(async () => {
        ...
        DeciblingAuction = await ethers.getContractFactory("DeciblingAuction");
        deciblingAuction = await upgrades.deployProxy(DeciblingAuction, [deciblingNFT.address, token.address, platformFeeRecipient.address]);
        await deciblingAuction.deployed();

        // Set DeciblingNFT's auction contract
        await deciblingNFT.connect(owner).setAuctionContractAddress(deciblingAuction.address);
    });
