...
contract("DeciblingStaking", () => {
  ...
  describe("Basic features on default pool", () => {
    beforeEach(async () => {
      ...
      const Reserve = await ethers.getContractFactory("DeciblingReserve");
      this.reserve = await upgrades.deployProxy(Reserve, [this.token.address], {
        initializer: "initialize",
      });
      ...
      //set contracts
      await this.reserve.setStakingContract(this.staking.address);
      await this.staking.setReserveContract(this.reserve.address);
    });
    ...
    it("stake()", async () => {
      ...
      expect(
        await this.token
          .connect(user1)
          .approve(this.staking.address, ONE_THOUSAND)
      );
      expect(
      ...
      //pool balance
      expect(await this.token.balanceOf(this.staking.address)).to.equal(
        ONE_THOUSAND
      );
      //user1 balance
      expect(await this.token.balanceOf(user1.address)).to.equal(
        ONE_BILLION.sub(ONE_THOUSAND)
      );
    }),
    ...
    describe("unstake cases", async () => {
      beforeEach(async () => {
        ...
        it("unstake()", async () => {
          await this.token.transfer(this.reserve.address, ONE_BILLION);
          // unstake
          expect(
            await this.staking
              .connect(user1)
              .unstake(defaultPoolId, ONE_THOUSAND)
          );
        ...
        it("unstake() with value larger than staked amount", async () => {
          ...
          await expect(
            this.staking
              .connect(user1)
              .unstake(defaultPoolId, ONE_THOUSAND.add(ONE_THOUSAND))
          ).to.be.revertedWith(
      ...
      describe("updatePoolOwner cases", async () => {
        beforeEach(async () => {
          ...
          it("updatePoolOwner()", async () => {
            ...
            await this.staking
              .connect(user1)
              .updatePoolOwner([], poolId, user2.address);
          ...
          it("updatePoolOwner() with invalid id", async () => {
            await expect(
              this.staking
                .connect(admin)
                .updatePoolOwner([], "invalid_id", user2.address)
            ).to.be.revertedWith("DeciblingStaking: this pool is not exist");
          }),
          it("updatePoolOwner() with invalid address", async () => {
            ...
            await expect(
              this.staking
                .connect(user1)
                .updatePoolOwner([], poolId, ZERO_ADDRESS)
      ...
      describe("hardCap cases", async () => {
        beforeEach(async () => {
          ...
          expect(
            await this.staking.pools("decibling_pool").owner,
            admin.address
          );
        }),
          it("default pool hardcap", async () => {
            ...
            assert.equal(
              (await this.staking.pools(poolId)).hardCap,
              10_000_000 * 1e18
            );
          });
      }),
      describe("payout cases", async () => {
        beforeEach(async () => {}),
          it("defaultPool payout 1 stake", async () => {
            ...
            await this.token
              .connect(user1)
              .approve(this.staking.address, 5_000_000);
            await this.staking.connect(user1).stake(poolId, 5_000_000);

            let depositTime = (
              await this.staking.stakers(poolId, user1.address)
            ).depositTime;

            // fast forward time to start of auction
            await ethers.provider.send("evm_setNextBlockTimestamp", [
              depositTime
                .add(depositTime, BigNumber.from(5 * 86400))
                .toNumber(),
            ]);
            ...
            console.log(
              await this.staking
                .connect(user1)
                .payout(poolId, user1.address, false)
            );
          }),
          it("newPool payout 1 stake", async () => {
            ...
            await this.token
              .connect(user1)
              .approve(this.staking.address, TEN_MILLION);
            ...
            // fast forward time to start of auction
            await ethers.provider.send("evm_setNextBlockTimestamp", [
              depositTime.toNumber() + 365 * 86400,
            ]);
            await ethers.provider.send("evm_mine");
          ...
          it("newPool payout 2 stakes", async () => {
            ...
            await this.token
              .connect(user1)
              .approve(this.staking.address, ONE_MILLION);
            ...
            // fast forward time
            await ethers.provider.send("evm_setNextBlockTimestamp", [
              depositTime.toNumber() + 10 * 86400,
            ]);
            await ethers.provider.send("evm_mine");

            let payout = await this.staking
              .connect(user1)
              .payout(poolId, user1.address, false);
            let age = (poolInfo.r / 100) * (10 / 365);
            let rate = Math.pow(Math.exp(1), age);
            let profit = ONE_MILLION * rate - ONE_MILLION;
            expect((payout / 1e18).toFixed(5)).to.equal(
              (profit / 1e18).toFixed(5)
            );
            ...
            depositTime = (await this.staking.stakers(poolId, user1.address))
              .depositTime;
